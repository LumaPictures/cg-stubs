# mypy: disable-error-code="misc, override, attr-defined, no-redef, assignment"

import QT4FormWidgets.ValuePolicy as ValuePolicy
from _typeshed import Incomplete
from typing import ClassVar

class PythonDictValuePolicy(ValuePolicy.AbstractValuePolicy):
    RESERVED_NAMES: ClassVar[set] = ...
    def __init__(self, name, dictionary, parent: Incomplete | None = ..., childOrderList: Incomplete | None = ..., childHintDict: Incomplete | None = ...): ...
    def _PythonValuePolicy__confirmChildHintDict(self, childHints): ...
    def _PythonValuePolicy__confirmOrderList(self, orderList): ...
    def getChildByName(self, name): ...
    def getChildren(self): ...
    def getName(self): ...
    def getNumChildren(self): ...
    def getType(self): ...
    def getWidgetHints(self): ...

class PythonGroupPolicy(ValuePolicy.AbstractValuePolicy):
    def __init__(self, name, parent: Incomplete | None = ...): ...
    def addChildPolicy(self, policy): ...
    def getChildByName(self, name): ...
    def getChildren(self): ...
    def getName(self): ...
    def getNumChildren(self): ...
    def getStateAppearance(self, state: set): ...
    def getType(self): ...
    def getWidgetHints(self): ...
    def removeChildPolicy(self, policy): ...
    def setWidgetHints(self, hints): ...

class PythonValuePolicy(ValuePolicy.AbstractValuePolicy):
    RESERVED_NAMES: ClassVar[set] = ...
    def __init__(self, name, dictionary, parent: Incomplete | None = ..., childOrderList: Incomplete | None = ..., childHintDict: Incomplete | None = ...): ...
    def _PythonValuePolicy__confirmChildHintDict(self, childHints): ...
    def _PythonValuePolicy__confirmOrderList(self, orderList): ...
    def getChildByName(self, name): ...
    def getChildren(self): ...
    def getName(self): ...
    def getNumChildren(self): ...
    def getType(self): ...
    def getWidgetHints(self): ...

class _ArrayPythonPolicy(ValuePolicy.AbstractValuePolicy):
    def __init__(self, name, valueList, parent): ...
    def _ArrayPythonPolicy__buildChildren(self): ...
    def getArrayChild(self, index): ...
    def getArraySize(self): ...
    def getName(self): ...
    def getType(self): ...
    def getWidgetHints(self): ...
    def setArraySize(self, size): ...

class _NumberPythonPolicy(_StringPythonPolicy):
    Types: ClassVar[tuple] = ...

class _StringPythonPolicy(ValuePolicy.AbstractValuePolicy):
    Types: ClassVar[tuple] = ...
    def __init__(self, name, dictionary, parent): ...
    def getName(self): ...
    def getType(self): ...
    def getValue(self): ...
    def getWidgetHints(self): ...
    def setValue(self, value, final: bool = ..., emit: bool = ...): ...