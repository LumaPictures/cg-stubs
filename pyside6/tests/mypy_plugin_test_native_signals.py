# GENERATED by tests/gen_mypy_plugin_cases.py from tests/test_native_signals.py -- do not edit by hand.
...
def test_connect_accepts_the_longest_signature() -> None:
    ...
    button.clicked.connect(r.one)
    button.clicked.connect(slot_checked)
    ...
    r.calls.clear()
    assert button.clicked.disconnect(slot_checked) is True
    assert button.clicked.disconnect(r.one) is True
    button.click()
    assert r.calls == [()]
...
def test_connect_signal_to_signal() -> None:
    ...
    button.clicked.connect(model.bool_sig)
    button.click()
    assert r.calls == [(False,)]
...
def test_connect_three_signatures() -> None:
    ...
    model.layoutChanged.connect(r.one)
    model.layoutChanged.connect(r.two)
...
def test_connect_qobject_destroyed() -> None:
    ...
    obj.destroyed.connect(slot_object)
...
def test_connect_qaction_triggered() -> None:
    ...
    action.triggered.connect(slot_checked)
    action.triggered.connect(r.one)
...
def test_emit_required_arguments_are_still_checked() -> None:
    ...
    # the only error left.
    process.finished.emit("0")  # type: ignore[arg-type]
    with pytest.raises(TypeError):
...
def test_emit_optional_arguments_are_still_checked() -> None:
    ...
    reply.errorOccurred.emit(error)
